...
try:
    ...
    import subprocess
    subprocess.run(["pip", "install", "open3d"], check=True)
    import open3d as o3d
...
vqvae.load_state_dict(torch.load(vqvae_path, map_location=torch.device('cpu'))["model_state_dict"])
...
mask_git.load_state_dict(torch.load(maskgit_path, map_location=torch.device('cpu'))["model_state_dict"])
