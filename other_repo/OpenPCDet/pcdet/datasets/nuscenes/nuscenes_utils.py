...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam=False, replace_with_mine_path=None):
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        #### I replace the original lidar and boxes with mine
        #replace_with_mine = False
        if replace_with_mine_path is not None:
            # root = "/home/shinghei/lidar_generation/OpenPCDet/data/nuscenes/v1.0-mini"
            # which_gen = "has_obj" #raw
            # path = f"/home/shinghei/generated_lidar/{which_gen}/v1.0-mini/token2sample.pickle"
            # path = f"{root}/{which_gen}/v1.0-mini/token2sample.pickle"
            path = replace_with_mine_path
            ...
            ref_lidar_path, ref_boxes, my_annotation_tokens, sample_records = token2sample_dict[ref_sd_token]
        ...
        if not test:
            print("train/VALLLLLL ANNOTATION BEING CREATED RIGHT N O W++++++++")
        ...
        if sample['scene_token'] in train_scenes:
            print("HELLLO train scene******************************************************")
            train_nusc_infos.append(info)
        else:
            print("HELLLO  val scene******************************************************")
