...
def get_my_data(lidar_token):
    ...
    return ref_lidar_path, ref_boxes, my_annotation_tokens, sample_records,ann_info_list
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False) -> EvalBoxes:
    ...
    for sample_token in tqdm_package.tqdm(sample_tokens, leave=verbose):
        ...
        for i, sample_annotation_token in enumerate(sample_annotation_tokens):
            ...
            sample_annotation = ann_info_list[i]
            lidar_points = np.fromfile(ref_lidar_path, dtype=np.float32).reshape(-1, 5)[:,:3]
            box = my_boxes[i]
            ...
            print(box.name)
            #assert(sample_annotation["category_name"]==box.name)
...
def visualize_sample(nusc: NuScenes,
    ...
    ref_lidar_path, _, my_annotation_tokens, sample_records, ann_info_list = token2sample_dict[lidar_token]#get_my_data(lidar_token)
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10, with_cam=False):
    ...
    if TRAIN_WITH_MY_AUGMENT:
        print("########### CONCAT WITH ORIG DATA......")
        time.sleep(3)
        #assert(1==0)
        orig_train_nusc_infos, orig_val_nusc_infos = nuscenes_utils.fill_trainval_infos(
        data_path=data_path, nusc=nusc, train_scenes=train_scenes, val_scenes=val_scenes,
        ...
        val_nusc_infos = val_nusc_infos + orig_val_nusc_infos

    #assert(len(val_nusc_infos)!=0)
    print(f"...NOTICE after CONCAT!!!!!: len train infos: {len(train_nusc_infos)}, len val infos: {len(val_nusc_infos)}")
    time.sleep(3)
