...
class ptBEVnet(nn.Module):
    ...
    def forward(self, pt_fea, xy_ind, voxel_fea=None):
        ...
        # print("len(pt_fea): ", len(pt_fea))
        # print("first pc: ", pt_fea[0].shape, "   first ind: ", xy_ind[0].shape)
        #### print("second pc: ", pt_fea[1].shape, "   second ind: ", xy_ind[1].shape)
        
        # concate everything (all points and all grid indices)
        cat_pt_ind = []
        ...
        # print("cat_pt_ind: ", cat_pt_ind.shape)
        ...
        unq, unq_inv, unq_cnt = torch.unique(cat_pt_ind,return_inverse=True, return_counts=True, sorted=True, dim=0)
        # unq[unq_inv] reconstructs the original tensor
        # unq: ( num_unique, 3)
        # unq inv: (num_unique,)
        # unq_cnt: (num_unique,)
        # print(f"unq: {unq}")
        # print(f"unq inv: {unq_inv}")
        # print(f"unq_cnt: {unq_cnt}")
        
        #device = torch.device('cuda', cur_dev)
        ...
        #print("cat_pt_fea after subsampling: ", cat_pt_fea.shape)
        ...
        #print(f"after point net: {pooled_data.shape}")
        ...
        # print(f"proceesed pooled data: {processed_pooled_data.shape}")
        # print(self.pt_fea_dim)
        # print(f"out data dim: {out_data_dim}")
       
        out_data = torch.zeros(out_data_dim, dtype=torch.float32).to(cur_dev)
        out_data[unq[:,0],unq[:,1],unq[:,2],:] = processed_pooled_data
        #print("!!!!!!!", out_data[unq[:,0],unq[:,1],unq[:,2],:].shape)
        out_data = out_data.permute(0,3,1,2) #(B,fea_dim,H,W)
        #print(f"*** out_data original:", out_data.shape)
        ...
        print("**** net return data: ", net_return_data.shape)
