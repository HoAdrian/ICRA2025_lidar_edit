...
def train(dataloader, model, device, optimizer, epoch):
    ...
    for batch_idx, data_tuple in enumerate(dataloader):
        ...
        =confusion_matrix_wrapper(voxels_occupancy_has.reshape(-1).detach().cpu().numpy(), rec_x_has.reshape(-1).detach().cpu().numpy(), labels=np.arange(num_classes))
        TPs += TPs_
        FPs += FPs_
        FNs += FNs_
        TNs += TNs_
        ...
        num_rec, num_gt = vqvae.occupancy_ratio(rec_x_has, voxels_occupancy_has)
...
if __name__=="__main__":
    ...
    optimizer = torch.optim.Adam(vqvae.parameters(), lr=0.0001) #0.0005
    ...
    for epoch in range(start_epoch, start_epoch+args.num_epochs):
        ...
        if epoch%2==0:
            ...
            write_csv_rows("./figures/vqvae_trans/train_val_loss.csv", rows, overwrite=False)

            rows = [[curr_epoch, accuracy_tr[-1,1], accuracy_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_accuracy.csv", rows, overwrite=False)

            rows = [[curr_epoch, precision_tr[-1,1], precision_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_precision.csv", rows, overwrite=False)

            rows = [[curr_epoch, recall_tr[-1,1], recall_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_recall.csv", rows, overwrite=False)

            rows = [[curr_epoch, f1_score_tr[-1,1], f1_score_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_f1score.csv", rows, overwrite=False)

            rows = [[curr_epoch, specificity_tr[-1,1], specificity_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_specificity.csv", rows, overwrite=False)

            rows = [[curr_epoch, TPR_tr[-1,1], TPR_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_TPR.csv", rows, overwrite=False)

            rows = [[curr_epoch, FPR_tr[-1,1], FPR_te[-1,1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_FPR.csv", rows, overwrite=False)

            rows = [[curr_epoch, train_code_util[-1], test_code_util[-1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_code_alive.csv", rows, overwrite=False)

            rows = [[curr_epoch, train_code_uniformity[-1], test_code_uniformity[-1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_code_uniformity.csv", rows, overwrite=False)

            rows = [[curr_epoch, train_occ_ratio[-1], test_occ_ratio[-1]]]
            write_csv_rows("./figures/vqvae_trans/train_val_occ_ratio.csv", rows, overwrite=False)

    ##### print performance #####
    accuracy_tr, precision_tr, recall_tr, f1_score_tr, specificity_tr, TPR_tr, FPR_tr = compute_perf_metrics(train_TPs[:], train_FPs[:], train_FNs[:], train_TNs[:])  
    accuracy_te, precision_te, recall_te, f1_score_te, specificity_te, TPR_te, FPR_te = compute_perf_metrics(test_TPs[:], test_FPs[:], test_FNs[:], test_TNs[:])

    print("metric for all classes at final iteration: ")
    print(f"precision: {precision_te[-1, 1]}")
    print(f"recall: {recall_te[-1, 1]}")
    print(f"f1 score: {f1_score_te[-1, 1]}")
    print(f"sepcificity: {specificity_te[-1, 1]}")
    ...
    xs = [i for i in range(start_epoch, start_epoch+len(train_losses))]
    plot_xy(xs, ys_list=[train_losses, test_losses], labels_list=["train", "val"], title="train-validation loss", x_label="epoch", y_label="loss", name="train_val_loss", path=f"./figures/vqvae_trans", vis=False)
    plot_xy(xs, ys_list=[accuracy_tr[:,1], accuracy_te[:,1]], labels_list=["train", "val"], title="train-validation accuracies", x_label="epoch", y_label="accuracy", name="train_val_accuracy", path=f"./figures/vqvae_trans", vis=False)
    #plot_xy(xs, ys_list=[train_auprcs, test_auprcs], labels_list=["train", "val"], title="train-validation auprcs", x_label="epoch", y_label="auprc", name="train_val_auprc", path=f"./figures/vqvae_trans", vis=False)
    for class_i in range(num_classes):
        plot_xy(xs, ys_list=[precision_tr[:, class_i], precision_te[:, class_i]], labels_list=["train", "val"], title=f"precision of class {class_i}", x_label="epoch", y_label="precision", name=f"train_val_precision_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False)           
        plot_xy(xs, ys_list=[recall_tr[:, class_i], recall_te[:, class_i]], labels_list=["train", "val"], title=f"recall of class {class_i}", x_label="epoch", y_label="recall", name=f"train_val_recall_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False) 
        plot_xy(xs, ys_list=[f1_score_tr[:, class_i], f1_score_te[:, class_i]], labels_list=["train", "val"], title=f"f1 score of class {class_i}", x_label="epoch", y_label="f1 score", name=f"train_val_f1score_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False)   
        plot_xy(xs, ys_list=[specificity_tr[:, class_i], specificity_te[:, class_i]], labels_list=["train", "val"], title=f"specificity of class {class_i}", x_label="epoch", y_label="specificity", name=f"train_val_specificity_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False)   
        plot_xy(xs, ys_list=[TPR_tr[:, class_i], TPR_te[:, class_i]], labels_list=["train", "val"], title=f"true positive rate of class {class_i}", x_label="epoch", y_label="true positive rate", name=f"train_val_TP_rate_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False)                  
        plot_xy(xs, ys_list=[FPR_tr[:, class_i], FPR_te[:, class_i]], labels_list=["train", "val"], title=f"false positive rate of class {class_i}", x_label="epoch", y_label="false positive rate", name=f"train_val_FP_rate_class_{class_i}", path=f"./figures/vqvae_trans/class_{class_i}", vis=False)    