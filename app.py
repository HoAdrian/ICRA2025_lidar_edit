...
# 1. Tell LFS to track all .pcd
git lfs install
git lfs track "*.pcd"
git add .gitattributes

# 2. Un‑cache every .pcd that git currently tracks
git rm --cached $(git ls-files '*.pcd')

# 3. Re‑add them so they go through LFS
git add $(git ls-files '*.pcd')

# 4. Commit & push
git commit -m "Track .pcd via Git LFS"
git push
'''
...
class DemoApp:
    def __init__(self, allocentric_dict, full_obj_pc_path, train_dataset: list, val_dataset: list, mask_git: MaskGIT, voxelizer: Voxelizer, device, mode="spherical"):
    ...
    def init_from_dataset(self, k: int):
        ...
        ####### what we want
        dataset_obj_boxes_list = sample["bbox_list"] #5
        scene_points_xyz = sample["scene_points_xyz"]

        ###### for drivable surface visualization
        ground_points = sample["ground_points"]
        other_background_points = sample["other_background_points"]
...
dataset_repo = "Shing-Hei/Nuscenes-mini-subset"
model_repo = "Shing-Hei/Inpainting_model_LiDAR_EDIT"
object_library_repo = "Shing-Hei/foreground_object_pointclouds"

#### Download my dataset from Hugging Face
print("loading dataset...")
train_dataset_path = hf_hub_download(dataset_repo, "train_mini_data.pickle", repo_type="dataset")
val_dataset_path = hf_hub_download(dataset_repo, "val_mini_data.pickle", repo_type="dataset")

######## Load dataset nuscenes
...
#### Download my model from Hugging Face
print("loading model checkpoints ...")
maskgit_path = hf_hub_download(model_repo, "maskgit_trans_weights/epoch_44", repo_type="model")
vqvae_path = hf_hub_download(model_repo, "vqvae_trans_weights/epoch_60", repo_type="model")
blank_code_path = hf_hub_download(model_repo, "blank_code.pickle", repo_type="model")
...
obj_library_path = hf_hub_download(object_library_repo, "allocentric.pickle", repo_type="dataset")

with open(obj_library_path, 'rb') as handle:
...
obj_full_pc_paths = {"car":[], "truck":[], "bus":[]}
for name in ["car", "truck", "bus"]:
    pc_names = allocentric_dict[name][1]
    # print(pc_names)
    for pc_name in pc_names:
        full_pc_obj_path = hf_hub_download(object_library_repo, f"dense_nusc/{name}/{pc_name}", repo_type="dataset")
        print(full_pc_obj_path)
        obj_full_pc_paths[name].append(full_pc_obj_path)
...
vqvae.load_state_dict(torch.load(vqvae_path)["model_state_dict"])
...
mask_git.load_state_dict(torch.load(maskgit_path)["model_state_dict"])

#### get blank codes for blank code suppressing 

print("loading previously generated blank code")
with open(blank_code_path, 'rb') as handle:
...
demo = DemoApp(allocentric_dict, obj_full_pc_paths, train_dataset, val_dataset, mask_git, voxelizer, device, mode)
demo.launch()
