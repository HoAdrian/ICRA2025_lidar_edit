...
if __name__=="__main__":
    ...
    logging.basicConfig(filename="inpainting_message.log", 
    ...
    lidar_root_path_list = ["GT", "our", "cut"]
    ...
    for k in samples:
        ...
        points_masked_GT = voxels2points(voxelizer, masked_GT_occupancy, mode=mode)[0]
        points_masked_gen = voxels2points(voxelizer, masked_gen_occupancy, mode=mode)[0]
        points_masked_vqvae = voxels2points(voxelizer, masked_vqvae_occupancy, mode=mode)[0]
        points_masked_cut = voxels2points(voxelizer, masked_cut_occupancy, mode=mode)[0]

        print("num_points_GT", len(points_masked_GT))
        print("num_points_gen", len(points_masked_gen))
        print("num_points_vqvae", len(points_masked_vqvae))
        print("num_points_cut", len(points_masked_cut))


        points_list = [points_masked_GT, points_masked_gen, points_masked_cut]
        lidar_sample_token = dataset.lidar_sample_token
        root = "/home/shinghei/inpainting_point_cloud_dataset"
        for save_idx, save_folder_name in enumerate(["GT_no_obj", "our_no_obj", "cut_no_obj"]):
            pc_name = f'val_{lidar_sample_token}.bin'
            os.makedirs(os.path.join(root, save_folder_name), exist_ok=True)
            lidar_full_path = os.path.join(root, save_folder_name, pc_name)
            points = points_list[save_idx]
            assert(not os.path.exists(lidar_full_path))
            assert(points is not None)
            points.astype(np.float32).tofile(lidar_full_path)
