...
if __name__=="__main__":
    ...
    #insert_xyz_pos = np.array([-5, 30,-1])
    alpha = np.deg2rad(225) # radian, increase in allocentric angle
    for k in samples:
        k = 31
        ...
        pc_filenames = os.listdir(pc_path)
        pc_filenames.sort()
        pc_filename = pc_filenames[0] # select an arbitrary completed point cloud
        pc_full_path = os.path.join(pc_path, pc_filename)

        #### loading pcd file here
        # coordinate system of kitti's lidar point clouds is the same as nuscenes
        vehicle_pc = np.asarray(open3d.io.read_point_cloud(pc_full_path).points)
        
        ##### kitti and nuscenes coordinate system is different
        ...
        center = np.mean(vehicle_pc, axis=0)
        original_viewing_angle = compute_viewing_angle(cam2obj_vector=center[:2])
        center_polar = cart2polar(center[np.newaxis,:], mode=mode)
        current_viewing_angle = compute_viewing_angle(insert_xyz_pos[:2])
    
        ##### center the vehicle point cloud at its centroid defined by its bounding box
        scale=1
        ...
        # rotation_angle = -(current_viewing_angle - original_viewing_angle) # negative sign because gamma increases clockwise
        ...
        #### custom rotation on vehicle_pc
        rotation_angle = -(alpha) # negative sign because allocentric angle increases when object is rotated clockwise
        vehicle_pc = cart2polar(vehicle_pc, mode=mode)
        theta = vehicle_pc[:,1]
        theta = theta + rotation_angle
        theta[theta<0] += 2*np.pi
        ...
        car_vis_pos = open3d.geometry.TriangleMesh.create_sphere(radius=0.2)
        car_vis_pos.paint_uniform_color([1,0,0])  
        car_vis_pos.translate(tuple(insert_xyz_pos))
